...
export function registerSSHTools(api: OpenClawPluginApi, sshConfig: SSHConfig): void {
  ...
  api.registerTool({
    ...
    description:
      'Execute a command on the gateway server via SSH. Use with caution — prefer specialized tools (gateway_status, gateway_restart, gateway_logs, gateway_update) when possible.',
    parameters: {
      type: 'object',
      properties: {
        command: {
    ...
    parameters: {
      ...
      properties: {
        service: {
          type: 'string',
          description:
            'Optional service name to restart (e.g., "core", "envoy"). Restarts all if not specified.',
        },
      },
    },
    execute: async (_id: string, params: Record<string, unknown>) => {
    ...
    parameters: {
      ...
      properties: {
        service: {
          type: 'string',
          description:
            'Optional service name to get logs from (e.g., "core", "envoy"). Shows all if not specified.',
        },
        lines: {
          type: 'number',
