...
A monorepo for running an AR.IO Arweave gateway and building custom sidecar services. The gateway handles Arweave data, and sidecars extend functionality (payments, content processing, C2PA provenance, APIs, etc.).
...
7. **Turbo binary conflict** - `@ardrive/turbo-sdk` shadows turborepo; scripts use explicit path
...
Detailed project knowledge is stored in Serena memories:
...
| `x402_bundler_sidecar`      | Bundler-specific implementation notes            |
